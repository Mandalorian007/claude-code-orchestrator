...
Perform comprehensive testing and validation of a Next.js application running in an E2B sandbox. Tests all layers (database, API routes, pages) both internally (within sandbox) and externally (via public URL), ensuring the application is production-ready before user access.
...
- All layers (database, API routes, pages) must be validated
...
- Browser UI Testing must execute ALL user story workflows from the plan's `### 6. Browser UI Testing` section
...
5. **Page Validation (External)**
   ...
   - Check that pages can reach API routes
...
6. **Integration Validation (End-to-End)**
   - Test complete user flow through all layers:
     ...
     3. Page makes API call to route handler
     4. Route handler processes request and updates database
     5. Database stores data correctly
     6. Route handler returns data to page
     7. Page displays updated data
...
8. **Error Resolution (If Tests Fail)**
   ...
   - Debug the specific failure:
     - Check application logs: `sbx exec [SANDBOX_ID] "cat .next/server/app-paths-manifest.json"`
...
### ✅ API Routes Validation (Internal)
...
### ✅ API Routes Validation (External)
...
- **Page → API Route**: ✅ API calls successful
- **API Route → Database**: ✅ Data persisted correctly
- **Database → API Route**: ✅ Data retrieved successfully
- **API Route → Page**: ✅ Data displayed correctly
...
For EACH `### User Story Workflow <N>: <Feature Name>` from the plan's `### 6. Browser UI Testing` section, report:
...
- ✅ Build succeeded
- ✅ API routes responding (internal + external)
- ✅ Pages built and served properly
